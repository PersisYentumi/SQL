-- creating database Sloar_System--
CREATE DATABASE Solar_System;

-- Creating table solar_system_objects--
...
-- Alter table to accommodate large values--

ALTER TABLE solar_system_objects MODIFY COLUMN body VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN mean_radius VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN mean_radius_rel VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN volume VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN volume_rel VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN mass VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN mass_rel VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN density VARCHAR(50);
ALTER TABLE solar_system_objects MODIFY COLUMN surface_gravity VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN surface_gravity_rel VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN type_of_object VARCHAR(255);
ALTER TABLE solar_system_objects MODIFY COLUMN shape VARCHAR(255);

-- Querying table solar_system_objects--

SELECT * FROM solar_system_objects;

-- average, max and min values in the data--
-- average mean_radius of the body--

SELECT AVG(mean_radius) FROM solar_system_objects;

-- average of the mean_radius_rel of body--

SELECT AVG(mean_radius_rel) FROM solar_system_objects;

-- average volume of body--
SELECT AVG(volume) FROM solar_system_objects;

-- average volume_rel of the body--
SELECT AVG(volume_rel) FROM solar_system_objects;

-- average mass of the body--
SELECT AVG(mass) FROM solar_system_objects;

-- average mass_rel of the body--
SELECT AVG(mass_rel) FROM solar_system_objects;

-- average density of the body--
SELECT AVG(density) FROM solar_system_objects;

-- average surface gravity of the body--
SELECT AVG(surface_gravity) FROM solar_system_objects;

-- average surface_gravity_rel of the body--
SELECT AVG(surface_gravity_rel) FROM solar_system_objects;

-- Max value of mean_radius of the body--
SELECT MAX(mean_radius) FROM solar_system_objects;

-- Max value of mean_radius_rel of the body--
SELECT MAX(mean_radius_rel) FROM solar_system_objects;

-- Max value of volume of the body--
SELECT MAX(volume) FROM solar_system_objects;

-- Max vlaue of volume_rel of the body--
SELECT MAX(volume_rel) FROM solar_system_objects;

-- Max value of mass of the body--
SELECT MAX(mass) FROM solar_system_objects;

-- Max value of mass_rel of the body--
SELECT MAX(mass_rel) FROM solar_system_objects;

-- Max value of density of the body--
SELECT MAX(density) FROM solar_system_objects;

-- Max value of surface_gravity of the body--
SELECT MAX(surface_gravity) FROM solar_system_objects;

-- Max value of surface_gravity_ rel of the body--
SELECT MAX(surface_gravity_rel) FROM solar_system_objects;

-- Min value of mean_radius of the body--
SELECT MIN(mean_radius) FROM solar_system_objects;

-- Min value of mean_radius_rel of the body--
SELECT MIN(mean_radius_rel) FROM solar_system_objects;

-- Min value of volume of the body--
SELECT MIN(volume) FROM solar_system_objects;

-- Min value of volume_rel of the body--
SELECT MIN(volume_rel) FROM solar_system_objects;

-- Min value of mass of the body--
SELECT MIN(mass) FROM solar_system_objects;

-- Min value of mass_rel of the body--
SELECT MIN(mass_rel) FROM solar_system_objects;

-- Min value of density of the body--
SELECT MIN(density) FROM solar_system_objects;

-- Min value of surface_gravity of the body--
SELECT MIN(surface_gravity) FROM solar_system_objects;

-- Min value of surface_gravity_rel of the body--
SELECT MIN(surface_gravity_rel) FROM solar_system_objects;

-- What about those numbers per category in the data (using HAVING)--
SELECT type_of_object, COUNT(*) AS count_of_objects
FROM solar_system_objects
GROUP BY type_of_object
HAVING COUNT(*) > 0;

SELECT shape , COUNT(*) AS 	count_of_shape
FROM solar_system_objects
GROUP BY shape
HAVING COUNT(*) > 0;

SELECT body, SUM(volume) AS total_volume
FROM solar_system_objects
GROUP BY body
HAVING SUM(volume) > 100;

SELECT shape, AVG(density) AS average_density
FROM solar_system_objects
GROUP BY shape
HAVING AVG(density) > 1;

-- What ways are there to group the data values that don’t exist yet (using CASE)--
-- Grouping data using CASE--
SELECT
    CASE
        WHEN mean_radius_rel > 1 THEN 'Larger than Earth'
        WHEN mean_radius_rel = 1 THEN 'Same as Earth'
        ELSE 'Smaller than Earth'
    END AS size_category,
    COUNT(*) AS count_of_planets
FROM
    solar_system_objects
WHERE
    type_of_object = 'planet (terrestrial)' OR type_of_object = 'planet (gas giant)'
GROUP BY
    size_category;
    ...
    CASE
        ...
        WHEN density >= 3 AND density <= 5 THEN 'Medium Density'
        ELSE 'Low Density'
...
GROUP BY
    ...
    SELECT
    CASE
        WHEN surface_gravity_rel >= 1 AND surface_gravity_rel <= 1.2 THEN 'Low Gravity'
        WHEN surface_gravity_rel > 1.2 AND surface_gravity_rel <= 2 THEN 'Medium Gravity'
        ELSE 'High Gravity'
...
GROUP BY
    ...
    SELECT * FROM solar_system_objects
    WHERE shape = "round" AND density > 1;
    
    SELECT * FROM solar_system_objects
    WHERE type_of_object = "satellite of Uranus" AND volume > 10;
    
    SELECT * FROM solar_system_objects
    WHERE type_of_object= "planet(gas giant)" AND surface_gravity < 100;
    
    SELECT * FROM solar_system_objects
    WHERE type_of_object = "star" AND mean_radius > 100;
    
    -- filtering the table using OR--
    SELECT * FROM solar_system_objects
    WHERE shape = "round" OR shape = "unknown";
    
SELECT * FROM solar_system_objects
WHERE type_of_object = "planet (gas giant)"
OR type_of_object = "planet (terrestrial)";
    
 -- filtering table combining AND & OR--
SELECT * FROM solar_system_objects
WHERE (type_of_object = "planet (gas giant)" 
OR density > 5) 
AND mean_radius < 5;
...
SELECT * FROM solar_system_objects
WHERE (body = "Earth" OR shape = "round")
AND volume > "500"; 